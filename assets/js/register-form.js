...
if (step == 2) {
	if ($('input[name="register_form"]').val() == 'true') {
		...
		if ($('input[name="service_type[]"]:checked').length == 0) {
			...
			if (values) {
				sessionStorage.setItem("service_type", JSON.stringify(values));
				console.log(sessionStorage.getItem("service_type"));
			}
		}
	}
}
	...
	if(step==5) { 
		...
		let customer_company = sessionStorage.getItem("company_name");
		$("span#customer_company").html('').html(customer_company);

		let customer_years_in_business= sessionStorage.getItem("years_in_business");
		$("span#customer_years_in_business").html('').html(customer_years_in_business);
		...
		let customer_best_way = sessionStorage.getItem("step-4");
		...
		// ************* Service Types *******************
		let customer_service_type = sessionStorage.getItem("service_type");
		let service_type = JSON.parse(customer_service_type);
		var resservice = [];
		for (var i in service_type)
		resservice.push(service_type[i]);
